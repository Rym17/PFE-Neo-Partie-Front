                  ...
                  <v-row class="mt-2 mx-3">
                    <v-col cols="12" md="12">
                      ...
                      >
                        ...
                        <v-tab :key="1">{{
                          $t("F01030.expansion-panels.ep0102")
                        }}</v-tab>
                        <v-tab :key="2">{{
                          $t("F01030.expansion-panels.ep0103")
                        }}</v-tab>
                        <v-tab :key="3">{{
                          $t("F01030.expansion-panels.ep0104")
                        }}</v-tab>

                        <v-tab :key="4">{{
                          $t("F01030.expansion-panels.ep0105")
                        }}</v-tab>
                      </v-tabs></v-col
                    >
                  </v-row>
                  <v-tabs-items v-model="tab">
                    <v-tab-item :key="1" class="tab">
                      <v-row class="mt-2 mx-3">
                        <v-col cols="12" md="12"
                          ><neo-auto-complete
                            v-bind:label="$t('F01030.Fields.CF_JS_TRIBUNAL')"
                            v-model="notice.data.CF_JS_TRIBUNAL"
                            return-object
                            item-text="VALEUR"
                            item-value="ID"
                            :items="CF_JS_TRIBUNAL_ITEMS"
                            @change="Filter_CHAMBRE()"
                          /> </v-col
                        ><v-col cols="12" md="12"
                          ><neo-auto-complete
                            v-bind:label="$t('F01030.Fields.CF_JS_CHAMBRE')"
                      ...
                      <v-row class="mt-2 mx-3"
                        ><v-col cols="12" md="12"
                          ><neo-date-picker
                            locale="ar-TN"
                            v-bind:label="
                              $t('F01030.Fields.CF_JS_DATE_DEPOT_REQUETE')
                            "
                            v-model="
                              notice.data.CF_JS_DATE_DEPOT_REQUETE
                            "/></v-col
                        ><v-col cols="12" md="12"
                          ><neo-date-picker
                            locale="ar-TN"
                            v-bind:label="
                              $t('F01030.Fields.CF_JS_AUDIENCE_PREMIERE_AVACAT')
                            "
                        ...
                        <v-col cols="12" md="12"
                          ><neo-date-picker
                            locale="ar-TN"
                            v-bind:label="
                              $t('F01030.Fields.CF_JS_AUDIENCE_PREMIERE')
                            "
                            v-model="notice.data.CF_JS_AUDIENCE_PREMIERE"
                        /></v-col>
                        <v-col cols="12" md="12"
                          ><neo-select
                            v-bind:label="
                              $t('F01030.Fields.CF_JS_HEURE_AUDIENCE')
                            "
                      ...
                      <v-row class="mt-2 mx-3">
                        <v-col cols="12" md="12"
                          ><neo-text-field
                            ...
                            v-model="notice.data.CF_JS_NUM_AFFAIRE"/></v-col
                      ></v-row>
                      <v-row class="mt-2 mx-3">
                    ...
                    <v-tab-item :key="2" class="tab">
                      ...
                      >
                        <v-expansion-panel>
                          <neo-expansion-panel-header
                            v-bind:label="$t('F01030.expansion-panels.ep01')"
                          ></neo-expansion-panel-header>
                        ...
                        <v-expansion-panel>
                          <neo-expansion-panel-header
                            v-bind:label="$t('F01030.expansion-panels.ep02')"
                          ></neo-expansion-panel-header>
                          <v-expansion-panel-content>
                            <TD_JS_DEFENDEURS
...
import TD_JS_DEMANDEURS from "../../datacards/TD_JS_DEMANDEURS_POST.vue";
import TD_JS_DEFENDEURS from "../../datacards/TD_JS_DEFENDEURS_POST.vue";
...
export default {
  ...
  data: () => {
    return {
      ...
      c01_ITEMS: [
        { ID: "001", VALEUR: "مدعي" },
        { ID: "002", VALEUR: "مدعي عليه" },
        { ID: "003", VALEUR: "دخيل" },
        { ID: "004", VALEUR: "متداخل" },
      ],
      CF_JS_MATIERE_ITEMS: [
  ...
  methods: {
    async Filter_CHAMBRE() {
      let getJsonFromSPResponse = [];
      getJsonFromSPResponse = await NeoApi.getJsonFromSP(
        "sp_AP01_GetCHAMBRE_BY_TRIBUNAL",
        [
          {
            ParameterName: "sp_CODE_TRIBUNAL",
            SqlDbType: "22",
            Value: this.notice.data.CF_JS_TRIBUNAL.ID,
          },
        ]
      );

      this.CF_JS_CHAMBRE_ITEMS = getJsonFromSPResponse.Result;
      console.log("this.CF_JS_CHAMBRE_ITEMS ", this.CF_JS_CHAMBRE_ITEMS);
    },
    hideLeftContainer() {
    ...
    TD_JS_DEMANDEURS_update(DataTable) {
      this.notice.TD_JS_DEMANDEURS.row = DataTable.data;
      console.log(
        "updateDataTable | TD_JS_DEMANDEURS : ",
        this.notice.TD_JS_DEMANDEURS.row
      );
    },
    TD_JS_DEFENDEURS_update(DataTable) {
      this.notice.TD_JS_DEFENDEURS.row = DataTable.data;
      console.log(
    ...
    TD_JS_DEMANDE_RAPPORT_update(DataTable) {
      this.notice.TD_JS_DEMANDE.row = DataTable.data;
      console.log(
    ...
    async saveTraceNotice(chrono, success, message) {
      ...
      this.notice.data.CF_JS_CHRONO_AVOCAT = chrono;
      this.trace.c01 = this.user.data.cn;
      this.trace.c02 = "التفاصيل";
    ...
    async saveNotice() {
      ...
      this.notice.mapping.SP_SIGNATORY = this.user.data.cn;
      this.notice.mapping.SP_SUBJECT = "ملف المحامي";
      this.notice.data.CF_JS_LEXICON_AVOCAT = this.user.data.mainServiceId;
      ...
      if (this.ValidationResult.success == "true") {
        var chrono = this.ValidationResult.chrono;
        var success = "نعم";
        var message = "-";
